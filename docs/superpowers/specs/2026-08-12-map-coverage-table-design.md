...
Below `md` (`< 768px`) the dedicated ID and participants columns are removed through TanStack column visibility, leaving a three-column grid. The map cell includes a secondary metadata line with the participant count but does not repeat the hidden ID. Coverage and the expansion action remain visible and right-aligned. Coverage uses a 64 px column without extra horizontal cell padding. Actions uses a 44 px column, explicitly removes inherited horizontal table-cell padding, and keeps the same 44 × 44 px expansion control as the participant table. The table receives a consistent 8 px right inset at every breakpoint so the expansion control does not touch the card edge without increasing the action column itself. Long map names truncate without pushing the percentage or action out of the viewport. The remaining three-column header stays in the DOM and accessibility tree but is visually clipped with the standard screen-reader-only technique rather than `display: none`; at `md` it returns to normal table-header rendering. At exactly 768 px, verification expects the five-column desktop layout.
