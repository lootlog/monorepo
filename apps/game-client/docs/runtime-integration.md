...
globals. Application processors, feature hooks, and domain stores may not.
...
- `game.store` — hero identity, world, and map;
...
- `reconcileAppliedEvent()` updates only domains and IDs touched by an applied
  event. A `town` event increments `mapEpoch` and replaces map, NPCs, and others.
