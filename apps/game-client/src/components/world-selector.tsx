...
export const WorldSelector: FC<WorldSelectorProps> = ({
  ...
  const worldGroups = useMemo<ComboboxGroup[]>(() => {
    ...
    if (recent.length > 0) {
      groups.push({
        label: t("settings.worldSelector.recentGroup"),
        options: recent,
      });
    }
    ...
    if (rest.length > 0) {
      groups.push({
        label: t("settings.worldSelector.allGroup"),
        options: rest,
      });
    }

    return groups;
  }, [t, worlds, recentWorlds]);
  ...
  return (
    <Combobox
      ...
      groups={worldGroups}
      placeholder={t("settings.worldSelector.placeholder")}
      searchPlaceholder={t("settings.worldSelector.searchPlaceholder")}
      emptyText={t("settings.worldSelector.emptyText")}
      disabled={disabled}
      triggerClassName={cn(
