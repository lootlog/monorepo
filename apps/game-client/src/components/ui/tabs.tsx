...
function TabsTrigger({ className, ...props }: BaseTabs.Tab.Props) {
  return (
    <BaseTabs.Tab
      ...
      className={cn(
        "ll:data-[active]:bg-accent ll:data-[active]:text-accent-foreground ll:data-[active]:border-ring ll:text-[12px] ll:text-white ll-custom-cursor-pointer ll:border-solid ll:border-border ll:box-border ll:border ll:rounded-sm ll:py-1 ll:px-2 ll:bg-transparent ll:hover:bg-accent ll:transition-all ll:mt-1",
        className,
      )}
      {...props}
    />
