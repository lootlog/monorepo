  ...
  if (isPending) {
    return (
      <div className="ll:p-4 ll:text-center ll:text-sm ll:text-gray-400">
        ...
        <p>{t("timers.contextMenu.creating")}</p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <ContextMenuItem onClick={onPin} className="ll:mt-1">
        ...
        {isPinned ? t("timers.contextMenu.unpin") : t("timers.contextMenu.pin")}
      </ContextMenuItem>
      <ContextMenuItem onClick={isPinned ? onUnpinAll : onPinAll}>
        <Globe className="ll:h-4 ll:w-4 ll:mr-2" />
        {isPinned
          ? t("timers.contextMenu.unpinAll")
          : t("timers.contextMenu.pinAll")}
      </ContextMenuItem>
      <ContextMenuItem onClick={isHidden ? onShow : onHide}>
        ...
        {isHidden ? t("timers.contextMenu.show") : t("timers.contextMenu.hide")}
      </ContextMenuItem>
      <ContextMenuItem onClick={isHidden ? onShowAll : onHideAll}>
        <Globe className="ll:h-4 ll:w-4 ll:mr-2" />
        {isHidden
          ? t("timers.contextMenu.showAll")
          : t("timers.contextMenu.hideAll")}
      </ContextMenuItem>
      {canReset && (
        <ContextMenuItem onClick={onReset}>
          <RotateCcw className="ll:h-4 ll:w-4 ll:mr-2" />
          {t("timers.contextMenu.countFromStart")}
        </ContextMenuItem>
      )}
      {timersGrouping ? (
        ...
        canDelete && (
          ...
          >
            ...
            {t("timers.contextMenu.deleteTimer")}
          </ContextMenuItem>
        )
      )}
    </>
