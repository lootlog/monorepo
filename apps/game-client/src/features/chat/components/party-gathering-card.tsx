      ...
      >
        ...
        {(partyGathering?.minLvl ?? partyGathering?.maxLvl) && (
          <p className="ll:text-[10px] ll:text-gray-400">
            {t("settings.chat.partyGathering.levelLabel", {
              ...
              >
                {isPending ? (
                  <>
                    ...
                    {t("settings.chat.partyGathering.pending")}
                  </>
                ) : !meetsLevelReq ? (
                  t("settings.chat.partyGathering.requiredLevel", {
                    min: minLvl,
                    max: maxLvl,
                  })
                ) : (
                  t("settings.chat.partyGathering.join")
                )}
              </Button>
            );
          })()}
