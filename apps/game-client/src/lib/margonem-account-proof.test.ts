...
describe("margonem-account-proof", () => {
  it("creates per-socket, per-account tokens with a fresh nonce", () => {
    ...
    expect(tokenA).toMatch(/^lootlog:socket-1:20:[a-f0-9]{32}$/);
    expect(tokenB).toMatch(/^lootlog:socket-1:20:[a-f0-9]{32}$/);
  ...
  it("requests Margonem proof with POST form data and credentials", async () => {
    ...
    await expect(
      ...
      userId: "20",
      token: expect.stringMatching(/^lootlog:socket-1:20:[a-f0-9]{32}$/),
      ts: 1_700_000_000,
      validatedString: expect.stringMatching(
        /^20\+lootlog:socket-1:20:[a-f0-9]{32}\+1700000000$/,
      ),
      signatureBase64: "signature",
    });
  ...
  it("rejects proof for a different request token", async () => {
    const fetchFn = vi.fn(async () =>
      createJsonResponse({
        user_id: "20",
        token: "lootlog:socket-1:20:ffffffffffffffffffffffffffffffff",
        ts: 1_700_000_000,
        validatedString:
          "20+lootlog:socket-1:20:ffffffffffffffffffffffffffffffff+1700000000",
        signatureBase64: "signature",
      }),
    );
