/**
 * Generated by orval v8.17.0 🍺
 * Do not edit manually.
 * Lootlog API
 * The Lootlog API documentation
 * OpenAPI spec version: 1.0
 */
import type { UserLootlogPlayersCatchingGuildsResponseDtoOutputPlayersItemGuildsItem } from './user-lootlog-players-catching-guilds-response-dto-output-players-item-guilds-item.js';

export type UserLootlogPlayersCatchingGuildsResponseDtoOutputPlayersItem = {
  userId: string;
  accountId: string;
  characterId: string;
  guilds: UserLootlogPlayersCatchingGuildsResponseDtoOutputPlayersItemGuildsItem[];
};
