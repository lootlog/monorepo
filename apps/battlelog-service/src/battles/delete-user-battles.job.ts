...
export function createDeleteUserBattlesWorker(
  ...
  const worker = new Worker<DeleteUserBattlesJobData>(
    ...
    async (job: Job<DeleteUserBattlesJobData>) => {
      ...
      loggerWithContext.info("Processing delete-user-battles job", { userId });
      const result = await battleStore.deleteUserBattles(userId);
      loggerWithContext.info("Completed delete-user-battles job", {
        userId,
  ...
  worker.on("failed", (job, error) => {
    loggerWithContext.error("Delete-user-battles job failed", {
      jobId: job?.id,
