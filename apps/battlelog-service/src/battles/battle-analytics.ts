...
import type { QueryBattleAnalyticsQuery } from "./schemas/query-battle-analytics.schema.js";
import type { QueryBattleStatisticsQuery } from "./schemas/query-battle-statistics.schema.js";
import type {
  ProfessionWinRate,
...
export class BattleAnalytics {
  ...
  async getBattleAnalytics(
    query: QueryBattleAnalyticsQuery,
    userId: string,
  ): Promise<{
    totalBattles: number;
    wins: number;
  ...
  async calculateProfessionWinRate(
    query: QueryBattleStatisticsQuery,
    userId: string,
  ): Promise<ProfessionWinRate[]> {
    const levelFilter = `${query.minLevel || "any"}-${query.maxLevel || "any"}`;
    const phFilter = query.ph ? "ph" : "all";
    const matchmakingFilter = query.matchmaking ? "matchmaking" : "all";
  ...
  async getPhGrowthTimeSeries(
    ...
    let cumulativePh = 0;
    const result: PhGrowthDataPoint[] = filteredBattles.map((battle) => {
      const userWarrior = battle.warriors.find((w) =>
        characterIds.includes(w.originalId),
      );
  ...
  async invalidateAnalyticsCache(userId: string): Promise<void> {
    try {
      ...
      const redis = this.redisCache.getClient();
      ...
      this.loggerWithContext.warn("Failed to invalidate analytics cache", {
        userId,
        error,
      });
    }
  ...
  async getRatingGrowthTimeSeries(
    ...
    const result: RatingGrowthDataPoint[] = filteredBattles.map((battle) => {
      return {
        date: battle.createdAt.toISOString(),
  ...
  async getPlayerVsPlayerBattles(
    query: QueryBattleStatisticsQuery & { opponentId: string },
    userId: string,
