...
import { RedisCache } from "../redis/redis-cache.js";

type R2Config = BattlelogAppConfig["r2"];

export class BattleArchive {
  ...
  constructor(
    ...
    this.logger.info("R2 client initialized", {
  ...
  async uploadBattleData(battleId: string, data: unknown): Promise<void> {
    try {
      ...
      this.logger.error("Failed to upload battle data", {
        battleId,
        error,
      });
      throw error;
  ...
  async getBattleData<T>(battleId: string): Promise<T> {
    try {
      ...
      this.logger.error("Failed to retrieve battle data", {
        battleId,
        error,
      });
      throw error;
  ...
  async deleteBattleData(battleId: string): Promise<void> {
    try {
      ...
      this.logger.info("Battle data deleted successfully", { battleId });
    } catch (error) {
      this.logger.error("Failed to delete battle data", {
        battleId,
        error,
      });
      throw error;
  ...
  async deleteBattleDataBatch(battleIds: string[]): Promise<void> {
    ...
    if (failed.length > 0) {
      this.logger.warn("Failed to delete some R2 objects", {
  ...
  private async cacheData(battleId: string, data: string): Promise<void> {
    try {
      ...
      this.logger.warn("Failed to cache battle data", {
        battleId,
        error,
      });
    }
  ...
  private async updateLRU(battleId: string): Promise<void> {
    try {
      ...
      this.logger.warn("Failed to update R2 LRU cache", {
        battleId,
        error,
      });
    }
  ...
  private async enforceLRULimit(): Promise<void> {
    try {
      ...
      this.logger.warn("Failed to enforce R2 cache LRU limit", {
        error,
      });
    }
  }
  ...
  private async deleteCachedData(battleId: string): Promise<void> {
    try {
      ...
      this.logger.warn("Failed to delete cached battle data", {
        battleId,
        error,
      });
    }
