...
export function prepareApiKeyEventVisibility(event: ServerEvent) {
  const room =
    event.type === "party-ready-room.updated"
      ? readSharedRoom(event.data.payload)
      : Option.none();
...
export const canReadApiKeyEvent = (
  session: SessionData,
  event: ServerEvent,
): boolean => prepareApiKeyEventVisibility(event)(session);
