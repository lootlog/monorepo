...
export class PaginationService {
  ...
  private async getPreviousCursor(
    ...
    const previousWindow = await this.drizzle.run(
      this.drizzle.db.query.battles.findMany({
        where: {
          RAW: (table: typeof battles) => {
            const base = whereBuilder(table);
  ...
  private async getEstimatedCount(where: SQL | undefined): Promise<number> {
    try {
      if (!where) {
        const result = await this.drizzle.run(
          this.drizzle.db.execute<{ estimated_count: string }>(sql`
      ...
      const result = await this.drizzle.run(
        this.drizzle.db.select({ count: count() }).from(battles).where(where),
      );
