...
const getGuildRoleHexColor = (role: GuildRole) =>
  role.color === 0
    ? null
    : `#${role.color.toString(16).padStart(6, "0").toUpperCase()}`;

const createTemplateEditorNodes = (value: string, roles: GuildRole[]) => {
...
const serializeTemplateEditorValue = () =>
  $getRoot()
    .getChildren()
    .map((paragraphNode) => {
...
const removeTemplateTokenNode = (tokenNode: LexicalNode) => {
...
const getBackspaceTemplateTokenNode = () => {
...
const SCHEDULED_MESSAGE_VARIABLE_KEYS = new Set(["ruleName", "scheduledFor"]);
...
export const NotificationTemplateEditor = ({
  ...
  useEffect(() => {
    const highlightedSuggestion = filteredSuggestions[highlightedSuggestionIndex];
  ...
  const timerPresets = [
    {
      ...
      value: "**{{npcName}}** spawns between {{minSpawnTime}} – {{maxSpawnTime}}",
    },
    {
      key: "detailed",
    ...
    <div className="space-y-3">
      ...
      <LexicalComposer
        initialConfig={{
          ...
          nodes: [NotificationTemplateRoleNode, NotificationTemplateVariableNode],
          onError: (error) => {
            throw error;
          },
        }}
                  ...
                  <Command>
                    <ScrollArea className="h-64">
                      <CommandList className="h-full max-h-none">
                        <CommandEmpty>
                          {t("settings.notifications.templateEditor.emptySuggestions")}
                        </CommandEmpty>
                        <CommandGroup>
