...
type ToolbarItem = ToolbarControl | { separator: string };
...
export const useGuildDocEditorToolbarModel = () => {
  ...
  const openLinkDialog = () => {
    ...
    savedSelection.current = nextSelection;
  ...
  const controls: ToolbarItem[] = [
    ...
    ...(tableControls.length > 0
      ? ([{ separator: "table" }, ...tableControls] as const)
      : []),
  ];
