...
async function truncateTimersState(
  ...
  try {
    ...
    await truncateTimersState(database, attempt + 1);
  }
}
...
describe("Timers E2E Tests (Whitelist)", () => {
  ...
  beforeEach(async () => {
    await truncateTimersState(database);
    const keys = (
      await Promise.all(
  ...
  describe("POST /timers/auto", () => {
    it("should reject invalid auto timer payload", async () => {
      ...
      await expect(database.timer.count()).resolves.toBe(0);
    });
    ...
    it("should let another request take over when the first burst owner fails before creating a timer", async () => {
      ...
      const upsertSpy = vi.spyOn(database.timer, "upsert");
    ...
    it("should submit one timer per whitelisted guild when 10 users submit the same NPC concurrently", async () => {
      const guilds = await Promise.all([
        database.guild.create({ data: TEST_GUILDS.GUILD_1 }),
        database.guild.create({ data: TEST_GUILDS.GUILD_2 }),
  ...
  describe("PATCH /guilds/:guildId/timers/:timerIdentifier/reset", () => {
    it("should reset an existing timer by timer key", async () => {
      ...
      const timer = await createTimerFixture(database, {
        guildId: guild.id,
        memberId: member.id,
        world: TEST_WORLD,
      ...
      const updated = await database.timer.findUnique({
        where: {
          timerId: {
            guildId: guild.id,
            world: TEST_WORLD,
  ...
  describe("DELETE /guilds/:guildId/timers/:timerIdentifier", () => {
    it("should delete an existing timer", async () => {
      ...
      const deletedTimer = await database.timer.findUniqueOrThrow({
        where: {
          timerId: {
            guildId: guild.id,
            world: TEST_WORLD,
