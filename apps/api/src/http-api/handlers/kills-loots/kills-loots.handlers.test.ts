...
const makeData = (overrides: Partial<KillsLootsData["Service"]> = {}) =>
  KillsLootsData.of({
...
const makeAuthorization = (
  overrides: Partial<KillsLootsAuthorization["Service"]> = {},
) =>
...
const services = (
  authorization: KillsLootsAuthorization["Service"],
  data: KillsLootsData["Service"],
) =>
  Layer.merge(
    Layer.succeed(KillsLootsAuthorization, authorization),
    KillsLootsData.layer(data),
  );
...
describe("Kills and Loots HttpApi handlers", () => {
  ...
  it("fails closed on missing authentication before accepting a loot", async () => {
    const denied = new KillsLootsAccessDenied({
      status: 401,
  ...
  it("maps a hidden or cross-Organization loot to not-found", async () => {
    ...
    expect(error).toBeInstanceOf(KillsLootsNotFound);
