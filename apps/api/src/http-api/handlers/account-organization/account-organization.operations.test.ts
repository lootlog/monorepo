...
const makeData = (
  overrides: Partial<AccountOrganizationData["Service"]> = {},
) =>
  AccountOrganizationData.of({
...
const makeAuthorization = (
  overrides: Partial<AccountOrganizationAuthorization["Service"]> = {},
) =>
...
const provideServices = (
  authorization: AccountOrganizationAuthorization["Service"],
  data: AccountOrganizationData["Service"],
  guildConfiguration: GuildConfigurationData["Service"] = makeGuildConfigurationData(),
) =>
  Layer.mergeAll(
    Layer.succeed(AccountOrganizationAuthorization, authorization),
    Layer.succeed(AccountOrganizationData, data),
...
describe("Users and Guilds HttpApi handlers", () => {
  ...
  it("does not cross the Organization boundary when the guild is not found", async () => {
    const notFound = new AccountOrganizationNotFound({
      status: 404,
