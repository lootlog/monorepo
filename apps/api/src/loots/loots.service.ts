...
export class LootsService implements OnModuleInit {
  ...
  async updateLoot(discordId: string, lootId: number, data: UpdateLootDto) {
    ...
    const mappedLootShareKeys = Object.keys(mappedLootShare);

    if (mappedLootShareKeys.length === 0) {
      throw new BadRequestException(ErrorKey.MISSING_LOOT_SHARE_ITEM_OR_PLAYER);
    }

    if (mappedLootShareKeys.length < items.length) {
      this.logger.log({
        level: "warn",
        message:
        ...
        mappedItemsCount: mappedLootShareKeys.length,
