...
export const noServiceConstructorImportsRule = defineRule({
	meta: {
        ...
        defaultOptions: [{ nonServiceImports: [] }],
		messages: {
		...
		return {
			ImportDeclaration(node) {
				...
				for (const specifier of node.specifiers) {
                    ...
                    // Syntactic names cannot prove an import constructs an Effect service.
                    // Callers can identify reviewed pure factories in a file-scoped override.
                    if (context.options[0]?.nonServiceImports?.includes(importedName)) continue;
