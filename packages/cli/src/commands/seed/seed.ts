...
function isValidDevUserId(userId: string | undefined): userId is string {
  return !!userId && userId !== "xxx";
}
...
async function seedGuilds(count: number) {
  ...
  for (let i = 0; i < guilds.length; i++) {
    ...
    const useDevelopmentIds = isDevGuild && isValidDevUserId(devUserId);
...
async function seedLoots(count: number, guilds: any[]) {
  ...
  for (const loot of loots) {
    ...
    if (guilds.length > 0) {
      ...
      if (isDevGuild && isValidDevUserId(devUserId)) {
  ...
  for (const guild of guilds) {
    ...
    for (let i = 0; i < timerCount; i++) {
      ...
      try {
        ...
        console.warn(`Failed to create timer for NPC ${randomNpc.id}:`, error);
      }
    }
  }
